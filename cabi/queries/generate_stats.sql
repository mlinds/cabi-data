...
-- of these routes, join all that share the same topological elements
statsums as (
    select
        count(jointtab.st) as trip_count,
        sum(jointtab.pop) as total_trips,
        (topelem) [1] as edge_id
    from
        jointtab,
        -- return one row for each unique topo element in the route
        getTopoGeomElements(jointtab.topogeom) as topelem
    -- aggregate on the element ID
    group by
        topelem
)
-- for each topoID, get the actual geometry from the topo schema, combine it with the stats, write the results to the new table
insert into
